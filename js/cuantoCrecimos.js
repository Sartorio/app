...
var cantidadPreguntas 	= [ 1, 2, 3, 4, 5 ];
var cantidad 			= 5;
var delay 				= 200;
//------------------------------------------------------------------------------- On loads
...
var Preguntas = (function($) {
	function crearPreguntas() {
	  for ( var i=0; i<cantidad; i++ ) {
		$('<div>' + "<img src='img-cuantoCrecimos/img" + cantidadPreguntas[i] + ".png'>" + '</div>').data( 'numero', cantidadPreguntas[i] ).attr( 'id', 'card' + cantidadPreguntas[i] ).appendTo( '#contenedorPregunta' ).draggable( {
		  containment	: 'section',
...
var Respuestas = (function($) {
	function creaRespuestas() {
	  for ( var i=1; i<=cantidad; i++ ) {
		$('<div>' + '?' + '</div>').data( 'numero', i ).appendTo( '#contenedorRespuesta' ).droppable( {
		  accept		: '#contenedorPregunta div',
		  hoverClass	: 'hovered',
		  drop			: DropDrag.handleCardDrop
		});
		$( '.ui-droppable' ).attr( 'class' , i );
	  }
		//alert('Carga Respuestas');
	}
	return {
...
var DropDrag = (function($) {
	function handleCardDrop( event, ui ) {
	  ...
	  if ( numeroRespuesta == numeroPregunta ) {
		...
		respuestasCorrectas++;
		$( '.correct' ).css({ 'margin-left' : '-8px', 'margin-top' : '30px' });
		validarResp.validarOk( numeroRespuesta );
...
//------------------------------------------------------------------------------- Validar OK
var validarResp = (function($) {
	function validarOk ( value ){
		//alert('Imagen anclada en respuesta' + value );
		$( '.' + value ).fadeIn( delay );
	}
	return {
		validarOk : validarOk
