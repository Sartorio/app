...
var Preguntas = (function($) {
	function crearPreguntas() {
	  for ( var i=0; i<cantidad; i++ ) {
		$('<div>' + "<img src='img-enLaCiudadEnElCampo/img" + cantidadPreguntas[i] + ".png' id='imgCorrect-" + cantidadPreguntas[i] + "' >" + '</div>').data( 'numero', cantidadPreguntas[i] ).attr( 'id', 'card' + cantidadPreguntas[i] ).appendTo( '#contenedorPregunta' ).draggable( {
		  containment	: 'section',
